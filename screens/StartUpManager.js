...
import { deepCvtKeyFromSnakeToCamel, initWs, URLJoin, asyncGetJson } from "../components/modules/support";
...
const updateTalk = (token, states, dispatches) => {
  authAxios(token)
    ...
    .then(async res => {
      ...
      // connect wsChat
      const prevTalkTicketCollection = await asyncGetJson("talkTicketCollection");
        ...
        talkTickets
          ...
          .forEach(talkTicket => {
            if (prevTalkTicketCollection[talkTicket.worry.key]?.status?.key === "talking") {
              ...
              // トークが開始されていない
              dispatches.chatDispatch({ type: "UPDATE_TALK_TICKETS", talkTickets: [talkTicket] });
              initConnectWsChat(talkTicket.room.id, token, states, dispatches, talkTicket.worry.key);
            }
          });
        ...
        talkTickets
          ...
          .forEach(talkTicket => {
            if (prevTalkTicketCollection[talkTicket.worry.key].status.key === "talking") {
              ...
              dispatches.chatDispatch({ type: "OVERWRITE_TALK_TICKET", talkTicket: talkTicket });
            }
          });

        Object.keys(prevTalkTicketCollection).forEach(key => {
          if (!talkTickets.some(talkTicket => talkTicket.worry.key === key)) {
            dispatches.chatDispatch({ type: "REMOVE_TALK_TICKETS", talkTicketKeys: [key] });
          }
        });
      } else {
        // signup直後一回のみ
        dispatches.chatDispatch({ type: "UPDATE_TALK_TICKETS", talkTickets: talkTickets });
        talkTickets
          .filter(talkTicket => talkTicket.status.key === "talking")
          .forEach(talkTicket => {
            initConnectWsChat(talkTicket.room.id, token, states, dispatches, talkTicket.worry.key);
...
const handleChatMessage = (data, chatState, chatDispatch, token, talkTicketKey) => {
  if (data.type === "chat_message") {
    ...
    if (isMe) {
      ...
      if (offlineMsgIDs.indexOf(messageID) >= 0) {
        chatDispatch({ type: "APPEND_MESSAGE", talkTicketKey, messageID, message, isMe, time, token });
        chatDispatch({ type: "DELETE_OFFLINE_MESSAGE", talkTicketKey, messageID });
      } else {
      }
    } else {
      chatDispatch({ type: "APPEND_MESSAGE", talkTicketKey, messageID, message, isMe, time, token });
...
const _connectWsChat = (wsProps) => {
  const { roomID, token, states, dispatches, init, callbackSuccess, talkTicketKey } = wsProps;
  ...
  const wsSettings = {
    ...
    onmessage: (e, ws, isReconnect) => {
      ...
      switch (data.type) {
        case "auth":
          if (isReconnect) {
            dispatches.chatDispatch({ type: "RECONNECT_TALK", ws, talkTicketKey });
          } else {
        ...
        case "end_talk_alert":
          dispatches.profileDispatch({ type: "SET_ALL", profile: data.profile });
          dispatches.chatDispatch({ type: "APPEND_COMMON_MESSAGE", talkTicketKey, alert: true });
          break;

        case "end_talk_time_out":
          dispatches.profileDispatch({ type: "SET_ALL", profile: data.profile });
          dispatches.chatDispatch({ type: "END_TALK", talkTicketKey, timeOut: true });
          break;
      ...
      handleChatMessage(data, states.chatState, dispatches.chatDispatch, token, talkTicketKey);
    },
    onclose: (e, ws) => {
    },
  };
...
const initConnectWsChat = (roomID, token, states, dispatches, talkTicketKey) => {
  _connectWsChat({
    roomID,
    token,
    states,
...
const reconnectWsChat = (roomID, token, states, dispatches, talkTicketKey) => {
  if (roomID) {
    ...
    dispatches.chatDispatch({ type: "RESTART_TALK_ONLY_MESSAGE", talkTicketKey });
  }
}
