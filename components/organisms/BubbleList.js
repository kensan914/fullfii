...
import { StyleSheet, Dimensions, ScrollView, TouchableOpacity, Pressable, LayoutAnimation } from "react-native";
...
const Bubble = (props) => {
  ...
  const onPress = useCallback((e) => {
    ...
    if (e.nativeEvent?.pageX === posWhenPressInX.current && e.nativeEvent?.pageY === posWhenPressInY.current) {
...
const BubbleList = (props) => {
  ...
  const [shouldShiftRowList, setShouldShiftRowList] = useState(
    new Array(items.length >= limitLines ? limitLines : items.length).fill(false)
  );
  ...
  const shouldShiftRow = (rowIndex, rowLen) => (
    rowIndex % 2 == 0
  );
          ...
          {items2d.map((rowItems, i) => (
            <Block
              ...
              style={[
                ...
                // ↓列単位のずらし↓
                shouldShiftRowList[i] ? { paddingLeft: (diameter / 2) + margin, } : {},
              ]}
            >
              {rowItems.map((item, j) => (
                <Bubble
...
const splitAndDeepenList = (list, splitNum) => {
  ...
  return lengthList.map(len => (
    resultList.splice(0, len)
  ));
}
